...
window.title("My Calculator/Калькулятор")
...
def button_click(label): #Функция, которая будет выполняться при нажатии на любую кнопку
    ...
    if label == "C": #Кнопка С очищает поле
        ...
        try:
            ...
            elif solution=="False" or solution=="Ошибка!" or solution=="" or solution=="None":
                result = "0"
                count_text.set(result)
                llr.append(solution+"="+result)
                lasres=result
        ...
        count_text.set(solution)
            ...
            def save_file():
                        ...
                        f.close()
            def find_text():
                find_string = simpledialog.askstring('Найти', 'Что искать?')
                if find_string:
            ...
            def make_bold():
                try:
                    ...
                    messagebox.showwarning('Предупреждение', 'Сначала выделите текст!')
            def make_normal():
                try:
                    current_tags = text_fild.tag_names('sel.first')
            ...
            file_menu.add_command(label='Найти', command=find_text)
            file_menu.add_separator()
            ...
            format_menu_sub.add_command(label='Жирный', command=make_bold)
            format_menu_sub.add_command(label='Курсив', command=make_italic)
            format_menu_sub.add_command(label='Подчёркнутый', command=make_underline)
            format_menu_sub.add_command(label='Обычный', command=make_normal)
            format_menu_sub.add_separator()
            format_menu_sub.add_command(
                label='Сбросить форматирование',
                command=lambda: reset_formatting())
            view_menu.add_cascade(label='Форматирование', menu=format_menu_sub)
            
            view_menu.add_command(label='Статистика текста', command=show_stats)
            ...
            view_menu_sub.add_command(label='Морская', command=lambda: chenge_theme('ocean'))
            view_menu_sub.add_command(label='Лесная', command=lambda: chenge_theme('forest'))
            view_menu_sub.add_command(label='Лунная', command=lambda: chenge_theme('moon'))
            view_menu_sub.add_command(label='Кофейная', command=lambda: chenge_theme('coffee'))            
            ...
            font_menu_sub.add_command(label='Wingdings (символы)', command=lambda: chenge_fonts('Wingdings'))
            font_menu_sub.add_command(label='Webdings (иконки)', command=lambda: chenge_fonts('Webdings'))
            font_menu_sub.add_command(label='Zapf Dingbats', command=lambda: chenge_fonts('ZapfDingbats'))
            
            view_menu.add_cascade(label='Вид шрифта', menu=font_menu_sub)
            ...
            view_menu.add_cascade(label='Размер шрифта', menu=size_menu_sub)
...
create_button(b_frame, "ЧАВО", 6,0, lambda: button_click("Кор=**0.5;д.нац.=//. Math.pow(число,степень). Скобки закрывать!"))
...
create_button(b_frame, "МОДУЛЬ", 5,1, lambda: button_click("abs("))
create_button(b_frame, "D", 0,2, lambda: button_click("D"))
create_button(b_frame, "ВРЕМЯ", 6,1, lambda: button_click("Date"))
create_button(b_frame, "РЕЗУЛЬТАТЫ", 6,2, lambda: button_click(llr))
create_button(b_frame, "ФАКТОРИАЛ", 1,4, lambda: button_click("FACTORIAL"))
create_button(b_frame, "ЛОГАРИФМ", 7,0, lambda: button_click("LOGARIFM"))
create_button(b_frame, "ЗАПЯТАЯ", 4,2, lambda: button_click(","))
create_button(b_frame, "E", 7,2, lambda: button_click("2.7182"))
create_button(b_frame, ";", 7,4, lambda: button_click(";"))
create_button(b_frame, "КОРЕНЬ", 5,4, lambda: button_click("math.sqrt("))
create_button(b_frame, "КУБКОРЕНЬ", 6,4, lambda: button_click("math.cbrt("))
create_button(b_frame, "СОХРЗАПИСЬ", 3,4, lambda: button_click("СЗ"))
create_button(b_frame, "ВСТАВЗАПИСЬ", 4,4, lambda: button_click("ВЗ"))
create_button(b_frame, "ВЫЙТИ", 0,4, lambda: button_click(window.destroy()))
create_button(b_frame, "ПРОБЕЛ", 2,4, lambda: button_click(" "))
create_button(b_frame, "СТЕПЕНЬ", 6,3, lambda: button_click("math.pow("))
create_button(b_frame, "БЛОКНОТ", 7,1, lambda: button_click("БЛОКНОТ"))
...
create_button(b_frame, "РАВНО", 2,5, lambda: button_click("RAVNO"))
create_button(b_frame, "<=", 3,5, lambda: button_click("<="))
create_button(b_frame, ">=", 4,5, lambda: button_click(">="))
create_button(b_frame, "НЕ РАВНО", 5,5, lambda: button_click("!="))
create_button(b_frame, ":", 7,5, lambda: button_click(":"))
create_button(b_frame, "СВОЙВВОД", 6,5, lambda: button_click("ВВОД:"))
